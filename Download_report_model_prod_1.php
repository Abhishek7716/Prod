...
class Download_report_model extends Model
{
    ...
    function get_subsequent_contribution_csv($from_date = null, $to_date = null,$report_type = null){ 
      ...
      $subQuery = DB::table('user_address_details')   
      ->select('*')   
      ->orderByRaw("FIELD(address_type, 'P', 'C')")    
      ->groupBy('pran'); 
      ...
      // ->leftJoin($this->user_address_details, $this->users . '.pran', '=', $this->user_address_details . '.pran')
    	->leftJoinSub($subQuery, 'uad', function ($join) {$join->on('users.pran', '=', 'uad.pran');})  
