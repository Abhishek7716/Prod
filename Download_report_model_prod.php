...
class Download_report_model extends Model
{
   ...
   ->groupBy($this->pg_transaction_details . '.payment_trans_id') ;
      ...
      if ($from_date && $to_date) {
        $query->whereBetween($this->subsequent_contribution_details . '.created_on', [$from_date, $to_date]);
      }
      if($report_type){
...
//      //for subsequent  -- (end)
   ...
   ->groupBy($this->pg_transaction_details . '.payment_trans_id');
      ...
      if ($from_date && $to_date) {
        $query_sip->whereBetween($this->contribution_details_cycle . '.created_on', [$from_date, $to_date]);
      }
