        ...
        if($contr_details)
        {
            ...
            if($data['success'] === true)
            {
                ...
                $rows_affected = $s2s_response_model->update_pg_details(
                    ...
                    array('gst_invoice_no' => $gst_invoice_no, 'payment_status' => 'A', 'payment_compl_on' => time(), 'updated_on' => time())
                );
	...
	public function payment_response(Request $request, Insert_s2s_response_model $s2s_respose_model, Cams_lib $cams_lib, Nsdl_lib $nsdl_lib, Kfintech_lib $kfintech_lib)
    {
        ...
        if($contribution_type && $type)
        {
            ...
            if($contribution_type == 'SUBSEQUENT') 
            {
                ...
                $mode_of_payment = isset($data['paymentDetail'][0]['modeOfPayment']) ? $data['paymentDetail'][0]['modeOfPayment'] : null;
            ...
            else if($contribution_type == 'SIP')
            {
                ...
                if($data['success'] === true) 
                {
                    ...
                    if ($mcd_data['sip_frequency'] == 'Monthly') {
                        ...
                        $start_day = date('d', $mcd_data['start_date']);
                        $sip_date = strtotime($mcd_data['mandate_sip_date'] . date('M-Y', $cdc_data['sip_date']) . " +1 month");
                        if ($start_day > 15) {
                            // Convert mandate_sip_date and sip_date to Unix timestamps for comparison
                            $mandate_sip_date = strtotime($mcd_data['mandate_sip_date'] . date('M-Y', $cdc_data['sip_date']) . " +1 month");
                            $sip_date_timestamp = strtotime($cdc_data['sip_date']);  // Assuming sip_date is already in Unix timestamp
                            
                            // Calculate the difference in seconds between sip_date and start_date
                            $sip_new_date = $mandate_sip_date - $mcd_data['start_date'];  // Difference in seconds
                            print_r_custom($sip_new_date);die();
                            // Convert difference in seconds to days
                            $diff_in_days = round($sip_new_date / (60 * 60 * 24));
                    
                            // If the difference is less than 20 days, add 2 months to the SIP date
                            if ($diff_in_days < 20) {
